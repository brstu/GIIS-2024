...
class AddressBook(QWidget):
    ...
    def initUI(self):
        ...
        layout.addWidget(self.edit_button, 1, 2)  # Редактировать под Адрес
        layout.addWidget(self.delete_button, 2, 2)  # Удалить под Редактировать
        layout.addWidget(self.save_button, 3, 2)  # Сохранить под Найти
        layout.addWidget(self.load_button, 4, 2)  # Загрузить под Сохранить
        layout.addWidget(self.prev_button, 5, 0)
    ...
    def editAddress(self):
        ...
        if name and address and self.current_index >= 0 and self.current_index < len(self.addresses):
            ...
            print('Некорректные данные для редактирования адреса')
