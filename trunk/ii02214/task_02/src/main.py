...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.button_edit.clicked.connect(self.edit_or_select_contact)
    ...
    def edit_or_select_contact(self):
        selected_row = self.list.currentRow()
        if selected_row != -1:
            if self.button_edit.text() == "Edit":
                self.edit_contact(selected_row)
            else:
                self.select_contact(selected_row)

    def edit_contact(self, selected_row):
        name = self.input_name.text()
        address = self.input_address.text()
        if name and address:
            contact = {"name": name, "address": address}
    ...
    def select_contact(self, selected_row):
